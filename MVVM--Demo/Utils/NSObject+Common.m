...
+ (void)showHudTipStr:(NSString *)tipStr{
    if (tipStr && tipStr.length > 0) {
        if ([tipStr isEqualToString:@"The Internet connection appears to be offline."]) {
            tipStr = @"似乎已断开与互联网的连接。";
        }
        if ([tipStr isEqualToString:@"The request timed out."]) {
            tipStr = @"请求连接超时。";
        }
...
+ (BOOL) saveImage:(NSData *)image imageName:(NSString *)imageName inFolder:(NSString *)folderName
{
    ...
    if ([self createDirInCache:folderName]) {
        ...
        if ( isDir == YES && existed == YES )
        {
            isSaved = [image writeToFile:[directoryPath stringByAppendingPathComponent:imageName] options:NSAtomicWrite error:nil];
        }
        return isSaved;
    }else{
        return NO;
...
//        return NO;
//    }else{
        requestPath = [NSString stringWithFormat:@"%@_%@", @"", requestPath];
//    }
